...
getPDFinRange <- function(y.true, test, train, form, learner, learner.pars,
  ...
  weightMat <- classPdf(tgt, train, cleanTest, y.true, 
                        form, learner, learner.pars, predictor, predictor.pars)
  Ltest <- nrow(test)
...
classPdf <- function(tgt, train, test, y.true, form, learner, learner.pars,
                     predictor, predictor.pars){
  # input:
...
#  print('End binning')
  ...
  cv_class <- generate_bi_probs(bins_class, train, cv, form, learner, learner.pars,
                                predictor, predictor.pars);
