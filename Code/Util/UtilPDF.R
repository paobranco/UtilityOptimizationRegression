# function for obtaining the optimal prediction for a conditional density estimation 
# of a regression task according to a utility surface provided by the user 

source("AuxsClassPdf.R")
source("classPdf.R")

UtilPDF <- function(form, train, test,
  ...
  for (ex in 1:nrow(test)){
      optim[ex] <- y.true[which.max(resPDF[ex,])]
  }
